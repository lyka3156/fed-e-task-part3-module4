...
# 13 dayjs 时间转换
# 14. 部署
## 14.1 Strapi 部署到支持node环境的web服务  (后台api接口服务)
1. 配置Strapi数据库

`默认sqlite数据库`
- 默认是通过sqlite本地文件的形式存储数据库
- 线上推荐使用mysql,mongode,...
  - [配置方式](https://strapi.io/documentation/developer-docs/latest/concepts/configurations.html#database)
``` js
module.exports = ({ env }) => ({
...
2. 将本地strapi项目推送到github/gitlab上
...
ssh root@远程服务器ip
// 将远程仓库的代码拉到服务器中
git clone https://github.com/lyka3156/blog-backend-strapi.git
// 安装依赖
npm i 
// 构建strapi
npm run build
...
npm run start启动strapi服务的命令，当服务关闭就strapi服务就关闭了，我们需要持久化存活在后端
- 使用 pm2 在后台运行
  - pm2 把node应用跑在后台启动
``` sh
# 手动pm2启动
# pm2 start 后面跟启动的命令
# --name 是取一个名字
pm2 start npm -- run start --name blog-backend-strapi
...
[ucloud端口配置视频](https://www.bilibili.com/video/av456518401/)

## 14.2 gridsome 部署到静态文件的web服务    (前端静态站点服务器)

[vercel](https://vercel.com/login)
- [vercel怎么使用](https://blog.csdn.net/qq_38735649/article/details/105687032)

当strapi的数据改变时自动部署vercel的项目
1. 在vercel中创建 Deploy Hooks
[Deploy Hooks]("../../images/task2/DeployHooks.png")

2. 在服务器上部署的strapi项目中添加webhook
[Deploy Hooks]("../../images/task2/webhook.png")

3. 通过上面添加hooks之后，你在strapi项目中做任何数据操作都会部署vercel中的gridsome项目重新构建